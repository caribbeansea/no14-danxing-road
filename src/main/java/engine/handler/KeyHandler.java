...
import engine.component.GamePanel;
...
public class KeyHandler implements KeyListener
{
     ...
     * 目前支持的109个键
     */
    private final Map<Integer, Key> keymaps = newKeyMap(
            ...
            KeyEvent.VK_BEGIN, KeyEvent.VK_UNDEFINED, (int) KeyEvent.CHAR_UNDEFINED, KeyEvent.KEY_LOCATION_UNKNOWN,
            KeyEvent.KEY_LOCATION_STANDARD, KeyEvent.KEY_LOCATION_LEFT, KeyEvent.KEY_LOCATION_RIGHT, KeyEvent.KEY_LOCATION_NUMPAD
    );

    private Map<Integer, Key> newKeyMap(Integer... keycodes)
    {
        Map<Integer, Key> maps = Maps.newHashMap(keycodes.length);
        try
        {
            Key key = new Key();
     ...
     * 判断某个键是否被按下
     *
     * @param keycode {@link KeyEvent}中定义的按键
     * @return 如果被按下返回true， 反之false。
     */
    public boolean isDown(int keycode)
    {
        return getKey(keycode).isDown();
    }
